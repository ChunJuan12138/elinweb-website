...
export function StatCard({ value, label, icon }: StatCardProps) {
 const cardRef = useScrollReveal<HTMLDivElement>({ y: 40, duration: 0.6, start: "top 88%" });
  ...
  return (
    <div ref={cardRef} className="opacity-0 group relative overflow-hidden rounded-xl border border-white/10 bg-white/5 p-6 text-center backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:bg-white/10">
      ...
      <p ref={valueRef} className="relative text-3xl font-bold text-white md:text-4xl">{displayValue}</p>
